//
//  >>>  arduino_nano_usb_parallelport.ino <<<      T.Hebbeker  
//  
//  = Software to emulate parallel port (centronics port, D-SUB connector) via USB interface
//  Arduino Nano board must be complemented by electronics and female D-SUB connector
//   
//  Arduino Nano translates between notebook/PC and parallel port.
...
//  use board = "Arduino Nano"  (if this is appropriate)
//  and chose appropriate USB port
//
//  sets 8 output "data line bits" and reads 5 "status bits" of Arduino Nano, 
...
//      This is ok for Arduino Nano which runs on 5V (so output lines get 5V, and not 3.3V)
...
const int GPIO_data_line[8] = {2,3,4,5,6,7,8,9};
